...
- 在整个已接收 buffer 中扫描 report/ACK 帧头；找到帧头前存在的垃圾字节时，一次丢弃整段前缀。
- 尚未找到完整帧头时，仅保留末尾 3 字节，其余字节丢弃。这样可以保留跨两次 `receive_buf()` 到达的 4 字节帧头前缀。
- 读出 little-endian `len`，计算总长度 `4 + 2 + len + 4`。
- 如果总长度超出 256 字节 buffer，计一次 parse error，丢弃当前 4 字节帧头后重新扫描。
- 如果数据还没收齐，就等待下一次 `receive_buf()`。
- 收齐后检查与帧类型对应的尾部；尾不对同样计错并丢弃当前 4 字节帧头后重新扫描。
