...
{
    int ret;
    struct i2c_client *client = (struct i2c_client *)ap3216c_device.client;
    if (!client)
    {
    ...
    dev_info(&client->dev, "device opened\n");
    return 0;
}
...
static ssize_t ap3216c_read(struct file *filp, char __user *buf, size_t cnt, loff_t *off)
{
    ...
    ap3216c_write_regs(dev, AP3216C_SYSTEMCONG, &tmp, 1);

    ret = ap3216c_read_regs(dev, AP3216C_IRDATALOW, raw, sizeof(raw));
    if (ret)
        return ret;

    data[0] = ((unsigned short)raw[1] << 2) | (raw[0] & 0x03);        /* IR */
    data[1] = ((unsigned short)raw[3] << 8) | raw[2];                 /* ALS */
    data[2] = ((unsigned short)(raw[5] & 0x3f) << 4) | (raw[4] & 0x0f); /* PS */
