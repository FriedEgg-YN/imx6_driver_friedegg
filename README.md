...
- **src**：本地外置源码目录（Local Source Tree），便于脱离 Buildroot 系统进行独立驱动开发及内核迭代：
    ...
    - `linux-friedegg`：基于 NXP 官方源码栈，结合正点原子教程移植的 Linux 内核工程（含定制化 Device Tree 设备树）。
    - `uboot-friedegg`：基于 NXP 官方源码栈，针对本地开发板进行适配的 U-Boot 芯片平台引导程序。
...
- [x] AP3216C 环境传感器 I2C 驱动编写并跑通应用层测试。
    ...
    - [x] 当前中断链路有问题，测试app 中断触发模式无法正常使用
- [ ] 确认BSP external包管理自动增量编译边界，优化build_and_deploy脚本。
...
在使用默认的增量模式开发底层驱动时，工程利用了 Buildroot 的特权覆盖机制。当前实际生效的覆盖文件是 `bsp/local.mk`，其中通过 `LINUX_OVERRIDE_SRCDIR = $(TOPDIR)/../src/linux-friedegg` 告诉 Buildroot：内核源码不是从下载包解压出来的，而是直接使用工作区里的 `src/linux-friedegg`。
